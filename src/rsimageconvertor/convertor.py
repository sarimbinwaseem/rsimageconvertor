...
class Convertor:
    ...
    def compressOne(self, image, size, form="same"):
        ...
        if form not in ["same", "png", "jpg", "jpeg"]:
            ...
            while True:
                ...
                image.save(outname, quality=85, optimize=True)
                ...
                if finalSize <= size:
                    ...
                    if form == "same":
                        image.save(
                            os.path.join(directory, f"{imgName}E.{extension}"),
                            quality=85,
                            optimize=True,
                        )
                    else:
                        image.save(
                            os.path.join(directory, f"{imgName}E.{form}"),
                            quality=85,
                            optimize=True,
                        )
                    os.remove(outname)
    ...
    def convertOne(self, filePath) -> None:
...
if __name__ == "__main__":
    con = Convertor()
    con.compressOne("pexels-stephan-seeber-1261728.jpg", 1500, "jpg")
